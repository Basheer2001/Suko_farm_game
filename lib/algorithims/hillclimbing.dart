...
class HillClimbing {
  ...
  void travers(){
    ...
    while(q.isNotEmpty){
      ...
      if(!current.isvisited){
        ...
        Logic nextNode=Logic();
        current.nextStates.forEach((element) {
          if(lowestheuristic>getheuristic(element)){
            lowestheuristic=getheuristic(element);
            nextNode = element;
          }
        });
        q.add(nextNode);
  ...
  void getPath(Logic goal){
    int cost=1;
    int heuristic=getheuristic(goal);
    int tCost =0;
    List<Logic?>list=[];
    list.add(goal);
  ...
  int getheuristic(Logic startNode){
